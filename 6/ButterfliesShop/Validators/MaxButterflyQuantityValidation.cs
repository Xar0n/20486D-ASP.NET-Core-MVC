        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            if (butterfly.ButterflyFamily != null)
            {
                ...
                if (sumQuantity > _maxAmount)
                    return new ValidationResult(string.Format("Limit of butterflies from the same family in the store " +
                        "is {0} butterflies. Currently there are {1}", _maxAmount, quantity));
